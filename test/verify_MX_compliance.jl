...
@testset "MX specification compliance" begin
    ...
    @testset "Element Data Types" begin
        ...
        @testset "FP8" begin
            ...
            @testset "E4M3" begin
                E4M3 = Microfloat(1, 4, 3; bounded=true)
                ...
                @test_broken reinterpret(E4M3, 0b0_1111_110) == 2^8 * 1.75
                @test_broken reinterpret(E4M3, 0b1_1111_110) == -2^8 * 1.75
        ...
        @testset "FP6" begin
            ...
            @testset "E2M3" begin
                E2M3 = Microfloat(1, 2, 3; bounded=true)
                ...
                @test_broken reinterpret(E2M3, 0b0_11_111_00) == 2^2 * 1.875
                @test_broken reinterpret(E2M3, 0b1_11_111_00) == -2^2 * 1.875
            ...
            @testset "E3M2" begin
                E3M2 = Microfloat(1, 3, 2; bounded=true)
                ...
                @test_broken reinterpret(E3M2, 0b0_111_11_00) == 2^4 * 1.75
                @test_broken reinterpret(E3M2, 0b1_111_11_00) == -2^4 * 1.75
        ...
        @testset "FP4" begin
            ...
            @testset "E2M1" begin
                E2M1 = Microfloat(1, 2, 1; bounded=true)
                ...
                @test_broken reinterpret(E2M1, 0b0_11_1_0000) == 2^2 * 1.5
                @test_broken reinterpret(E2M1, 0b1_11_1_0000) == -2^2 * 1.5
    ...
    @testset "Scale Data Types" begin
        ...
        @testset "E8M0" begin
            E8M0 = Microfloat(0, 8, 0; bounded=true)

            @test Microfloats.bias(E8M0) == 127

            #@test_broken floatmax(E8M0) == floatmax(Float32) / 2

            @test !isfinite(reinterpret(E8M0, 0b11111111))

            @test isnan(reinterpret(E8M0, 0b11111111))

            @test_broken !iszero(reinterpret(E8M0, 0b00000000))
        end
